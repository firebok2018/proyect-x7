...
delimiter ;
call verificar_user('joses','12345678');
...
create procedure add_Platillos(nVen char(5),nomc char(5),x varchar(200),tipo varchar(50),des varchar(200),cant int(11),usrV char(5))
begin
	insert into platillos values(nVen,nomc,x,tipo,des,cant,usrV);
end $$
delimiter ;
#call add_Platillos('V0001','C0001','Extras','nombre apellido','dss',4,'U0002');
#call add_Platillos('V0002','C0001','nombre apellido','Extras','dss',4,'U0001');

delimiter $$
DROP PROCEDURE IF EXISTs update_Venta $$
create procedure update_Venta(nVen char(5),nomc char(5),x varchar(200),tip varchar(50),des varchar(200),cantd int(11),usrV char(5))
begin
	update platillos set numVenta=nVen,codCli=nomc,nomX=x,tipo=tip,descripcion=des,cant=cantd,nomV=usrV where numVenta=nVen and nomV=usrV;
end $$
delimiter ;

#call update_Venta('V0001','C0001','Menu','arroz conpato','bebidasdas',4,'WISDS');

delimiter $$
drop procedure if exists delete_venta $$
create procedure delete_venta(nv char(5),nom char(5))
begin 
	delete from platillos where numVenta=nv and nomV=nom ;
end $$
delimiter ;
call delete_venta('V0004');
