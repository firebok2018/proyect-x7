...
public class ConexionVentas {
	...
	public void delete_venta(Venta x){
		Connection con = null;
		CallableStatement sp= null;
		try {
			con= new Conexion().getConexion();
			...
			sp= con.prepareCall(u_sp);
			sp.setString(1,x.getCodigoVenta());
			sp.setString(2,x.getUserV());
			sp.executeQuery();
			con.commit();
			...
			System.out.println(e.getSQLState()+e.getErrorCode()+e.getLocalizedMessage()+e.getNextException()+e.getCause());
			JOptionPane.showMessageDialog(null, "El Codigo de venta "+x+" No se Encuentra en Registrado ! ",null,0);
		}finally {
			try {
				con.close();
				sp.close();
